...
function getWeatherByLocation() {
  ...
  function searchWeatherByCoordinates(latitude, longitude) {
    fetch(`http://api.openweathermap.org/data/2.5/weather?lat=${latitude}&lon=${longitude}&appid=${appId}&units=${units}`)
      .then((result) => {
        return result.json();
      })
      .then((res) => {
  ...
  // Call getWeatherByLocation to get weather for current location by default
